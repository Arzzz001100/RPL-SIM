...
const Laporan: React.FC<LaporanProps> = ({ onBack }) => {
  ...
  const [isSubmitting, setIsSubmitting] = useState<boolean>(false); // Proteksi double submit
  ...
  // Bersihkan semua sisa object URL saat komponen dibongkar (unmount)
  useEffect(() => {
    return () => {
  ...
  const handleRemoveFoto = (indexToRemove: number, e: React.MouseEvent) => {
    ...
    // Perbaikan 1: Revoke object URL untuk mencegah kebocoran memori (Memory Leak)
  ...
  const handleSimpanDraft = () => {
    if (!user.id) return alert("Sesi login tidak valid. Gagal menyimpan draf.");
  ...
  const handleKirimLaporan = async () => {
    if (!user || !user.id) return alert("Sesi Anda berakhir, silakan login ulang!");
    ...
    if (trimmedDeskripsi.length < 10) {
      return alert("Deskripsi terlalu pendek! Berikan keterangan minimal 10 karakter.");
    }

    // Anti-Spam & Keyboard Smash Protection
    ...
    // Kunci tombol kirim agar tidak bisa ditekan berkali-kali (Double Submit)
    setIsSubmitting(true);

    const formData = new FormData();
    formData.append("id_siswa", user.id.toString());
    ...
    try {
      ...
      if (res.ok) {
        // Bersihkan objek URL setelah sukses terkirim
        previews.forEach((url) => URL.revokeObjectURL(url));
        ...
        alert(`Gagal mengirim laporan: ${errorData.error || "Terjadi galat internal server."}`);
      }
    } catch (err) {
      console.error(err);
      alert("Terjadi kesalahan koneksi ke backend!");
    } finally {
      setIsSubmitting(false); // Buka kembali kunci pengiriman jika terjadi error
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      <nav className="px-10 py-5 bg-white border-b flex justify-between items-center shadow-sm">
        ...
        <button
          ...
          className="px-6 py-2 bg-gray-200 text-gray-700 rounded-full font-bold text-sm hover:bg-gray-300 transition-all disabled:opacity-50"
        >
          Kembali
        </button>
      </nav>
        ...
        <div className="bg-white w-full max-w-4xl rounded-[40px] shadow-2xl border border-blue-100 p-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-6">
              ...
              <div>
                ...
                <select
                  ...
                  disabled={isSubmitting}
                  className="w-full px-5 py-3 rounded-xl border border-blue-200 outline-none focus:ring-2 focus:ring-blue-500 font-bold text-blue-900 disabled:bg-gray-50"
                >
            ...
            <div className="space-y-6">
              <div>
                ...
                <div
                  onClick={() =>
                    !isSubmitting && fotos.length < 4 && fileInputRef.current?.click()
                  }
                  className={`h-32 border-2 border-dashed border-blue-200 rounded-2xl flex flex-col items-center justify-center bg-blue-50/30 text-blue-400 overflow-hidden transition-all text-center px-4 ${fotos.length < 4 && !isSubmitting ? "cursor-pointer hover:bg-blue-50" : "cursor-not-allowed opacity-70"}`}
                >
                  {previews.length > 0 ? (
                      ...
                      {previews.map((src, index) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] font-bold shadow-md hover:bg-red-600 transition-colors disabled:opacity-40"
                          >
                            ✕
                          </button>
                        </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  maxLength={1000}
                  disabled={isSubmitting}
                  className="w-full h-32 px-5 py-3 rounded-2xl border border-blue-200 outline-none focus:ring-2 focus:ring-blue-500 resize-none font-medium text-blue-900 disabled:bg-gray-50"
                ></textarea>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-6 mt-12">
            <button
              ...
              disabled={isSubmitting}
              className="flex-1 py-4 bg-blue-500 text-white font-black rounded-2xl shadow-lg uppercase text-sm tracking-widest transition-all active:scale-95 hover:bg-blue-600 disabled:opacity-50"
            >
              DRAF
            </button>
            <button
              ...
              disabled={isSubmitting}
              className="flex-1 py-4 bg-[#1e1b4b] text-white font-black rounded-2xl shadow-lg uppercase text-sm tracking-widest transition-all active:scale-95 hover:bg-black disabled:bg-gray-400"
            >
