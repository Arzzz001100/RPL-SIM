...
const Konsultasi: React.FC<Props> = ({ onBack, onSent }) => {
  ...
  useEffect(() => {
    let isMounted = true; // Proteksi antispam memory leak / race condition
      ...
      .then((data) => {
        if (isMounted) {
          ...
          if (data && data.length > 0) {
            setIdGuru(data[0].id_guru); // Sinkronisasi awal elemen pertama
          }
        }
      })
    ...
    return () => {
      isMounted = false; // Batalkan pembaruan state jika komponen di-unmount
    };
  }, []);

  const handleKirim = async () => {
    // Validasi Sesi Login Siswa
    if (!user || !user.id) {
      return alert("Sesi Anda telah berakhir. Silakan login kembali!");
    }

    if (!idGuru) {
      return alert("Silakan pilih guru pembimbing terlebih dahulu!");
    }
    ...
    if (day === 0 || day === 6) {
      return alert("Maaf, konsultasi tidak tersedia di hari Sabtu dan Minggu.");
    }

    // Validasi Jam Operasional (07:00 - 15:00) menggunakan manipulasi string yang aman
    const [jamStr, menitStr] = jam.split(":");
    const jamTerpilih = parseInt(jamStr, 10);
    const menitTerpilih = parseInt(menitStr, 10);
    ...
    if (
      ...
      return alert("Maaf, layanan konsultasi hanya tersedia pukul 07:00 - 15:00 WITA.");
    }

    // Daftar Hari Libur Nasional (2026)
    const holidayList = ["2026-05-01", "2026-05-13", "2026-06-01"];
    if (holidayList.includes(tanggal)) {
      return alert("Maaf, tanggal yang Anda pilih adalah hari libur nasional.");
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Gagal mengirim permintaan. Silakan coba beberapa saat lagi.");
      }
    } catch (error) {
